...
### 1. WriteCode handler
- [x] `backend/app/orchestrator/handlers/write_code.py`
- [x] Load approved artifacts: PRD, design_spec, tech_plan
- [x] Use `customtools` model profile (tool-heavy coding)

### 2. Code generation strategy
- [x] Generate code in structured chunks to stay within token limits:
...
- [x] Each chunk: separate LLM call with context from previous chunks
- [x] Assemble into a single `code_snapshot` artifact

### 3. Code snapshot format
- [x] `backend/app/schemas/code_snapshot.py`:
  ```python
...
- [x] After generating code snapshot, run basic validation:
...
- [x] On validation failure: return actionable remediation data in error

### 5. Security gate feedback loop
- [x] If `SecurityGate` fails and loops back to `WriteCode`:
...
## Definition of Done
- [x] Code snapshot contains a buildable set of files
- [x] Files match the tech plan's folder structure and API routes
- [x] Validation catches basic syntax/import errors
- [x] Security remediation loop produces patched code
- [x] Failures return actionable error messages
