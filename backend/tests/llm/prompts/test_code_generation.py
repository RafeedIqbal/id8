def test_build_chunk_prompts_with_template_context():
    ...
    assert "package_requirements" in system_prompt
    assert "DO NOT EVER output a replacement `package.json` file" in system_prompt
    assert "DO NOT choose versions" in system_prompt

    # Check user prompt assertions
    assert "package requirements for the **shared_foundation** phase" in user_prompt
